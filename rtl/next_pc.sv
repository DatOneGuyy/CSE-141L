...
logic [9:0] pc_p1 = current_pc + 10'b1;

//1 appended for unconditional jumps
logic [7:0] extended_flags = {1'b1, flags};
...
logic [8:0] one_hot_code = {jump_target[16:12], ltu, gtu, lts, gts};
logic no_jump = ~|one_hot_code;

logic [9:0] jump_address = {jump_target[17], jump_target[8:0]};
...
always_comb begin
    ...
    for (int i = 0; i < 9; i++) begin
        unique if (one_hot_code[i]) begin
            if (i < 7 & extended_flags[i]) new_pc = jump_address;
