...
$readmemb("../rtl/programs/multiply.bin", D1.instruction_memory_inst.mem);
    for(int loop_ct=0; loop_ct<itrs; loop_ct++) begin
	    ...
	    for(int k=0; k<16; k++) begin
	      if({D1.dm.core[64+4*k],D1.dm.core[65+4*k],D1.dm.core[66+4*k],D1.dm.core[67+4*k]} == Prod[k]) begin
	        $display("Pass! %d * %d = %d",Tmp[2*k+1],Tmp[2*k],Prod[k]);
	        test_ctr++;
          end
	      else begin
	        $display("Fail! %d * %d should = %d, actual = %d",Tmp[2*k+1],Tmp[2*k],Prod[k],{D1.dm.core[64+4*k],D1.dm.core[65+4*k],D1.dm.core[66+4*k],D1.dm.core[67+4*k]});
          end
        end
