...
logic [15:0] Dist, Min, Max;	// current, min, max Hamming distances
...
initial begin
    ...
    for (int r = 65; r < 256; r++) D1.dm.core[r] = 'd0; // preset DUT final Max to min possible 
    ...
    $readmemb("../test_files/test2_2.txt",D1.dm.core);
    Min = 'hffffffff;						 // start test bench Min at max value
    ...
    if (Max == {D1.dm.core[68], D1.dm.core[69]}) 
        ...
        $display("MAD  Max = %d, yours = %d", Max, {D1.dm.core[68], D1.dm.core[69]}); // your DUT put wrong answer into core[68:69]
    $display("Max pair = %d, %d", Max1, Max2);
    ...
    #100ns;
    $readmemh("../test_files/test3_2.txt", D1.dm.core);
    ...
    for (int k = 0; k < 16; k++)
        if ({D1.dm.core[64 + 4 * k], D1.dm.core[65 + 4 * k], D1.dm.core[66 + 4 * k], D1.dm.core[67 + 4 * k]} == Prod[k])
            ...
            $display("Boo! %d * %d should = %d, yours = %d", Tmp[2 * k + 1], Tmp[2 * k], Prod[k], {D1.dm.core[64 + 4 * k], D1.dm.core[65 + 4 * k], D1.dm.core[66 + 4 * k], D1.dm.core[67 + 4 * k]});    
